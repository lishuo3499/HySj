...
class VideoViewSet(viewsets.ModelViewSet):  # list video
    queryset = Video.objects.all()
    serializer_class = VideoSerializer


class VideoCommentPagination(PageNumberPagination):
    page_size = 20
...
class VideoFavDetail(APIView):  # 点赞数操作
...
class VideoForwardingDetail(APIView):  # 转发数操作
    def get_object(self, pk):
        try:
            return Video.objects.get(pk=pk)
        except Video.DoesNotExist:
    ...
    def get(self, request, pk, format=None):
        video = self.get_object(pk)
        video.forwarding_num += 1
        video.save()
        serializer = VideoSerializer(video)
        return Response(serializer.data)


class VideoCommentFavDetail(APIView):
    def get_object(self, pk):
        try:
    ...
    def get(self, request, pk, format=None):
        videocomment = self.get_object(pk)
        videocomment.fav_num += 1
        videocomment.save()
        serializer = VideoCommentSerializer(videocomment)
        return Response(serializer.data)


def Videocomment(request):
    comment = VideoComment()
    comment.user_id = request.POST['id']
